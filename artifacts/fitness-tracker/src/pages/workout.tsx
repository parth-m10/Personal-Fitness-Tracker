...
import { CheckCircle, Info, History, Timer } from "lucide-react";
import { motion } from "framer-motion";
import { useLocation } from "wouter";
import { TimerWidget } from "@/components/workout/TimerWidget";
...
const TIMED_TRACKING_TYPES = new Set(["timed", "duration", "warmup", "cardio"]);

function isTimedExercise(trackingType: string, category: string) {
  return TIMED_TRACKING_TYPES.has(trackingType) || category.toLowerCase() === "cardio";
}
...
export default function Workout() {
  ...
  // track which exercise cards have their timer panel open
      ...
      workoutDay.exercises.forEach((ed) => {
        initial[ed.exerciseId] = {
          ...
          actualDurationSeconds:
            ed.targetDurationSeconds?.toString() || ed.exercise.defaultDurationSeconds?.toString() || "",
          formQuality: "Good",
  ...
  const handleExChange = (exId: number, field: keyof ExerciseFormState, value: any) => {
    setExerciseForms((prev) => ({ ...prev, [exId]: { ...prev[exId], [field]: value } }));
  };

  const toggleTimer = (exId: number) => {
    setOpenTimers((prev) => ({ ...prev, [exId]: !prev[exId] }));
  };
  ...
  const handleFinish = () => {
    ...
    const exerciseLogs = workoutDay.exercises.map((ed) => {
      ...
      return {
        ...
        notes: f.notes || null,
        totalVolume:
          f.actualSets && f.actualReps && f.actualWeightKg
            ? Number(f.actualSets) * Number(f.actualReps) * Number(f.actualWeightKg)
            : null,
      };
    });
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <h1 className="text-3xl font-bold tracking-tight">
              Cycle {appState?.currentCycleNumber} &bull; Day {workoutDay.dayNumber}
            </h1>
      ...
      <div className="space-y-6">
        {workoutDay.exercises
          ...
          .map((ed, idx) => {
            ...
            const timed = isTimedExercise(ex.trackingType, ex.category);
            const prev = prevMap[ex.id];

            // Determine which timers are available for this exercise
            const exerciseTimerSec =
              ed.targetDurationSeconds ?? ex.defaultDurationSeconds ?? (timed ? 60 : null);
            const hasExerciseTimer = timed && exerciseTimerSec != null;
            const hasRestSetsTimer = !!ed.restBetweenSetsSec;
            const hasRestExTimer = !!ed.restBetweenExercisesSec;
            const hasAnyTimer = hasExerciseTimer || hasRestSetsTimer || hasRestExTimer;
            ...
            return (
              ...
              >
                <Card
                  className={`border-card-border overflow-hidden transition-all duration-300 ${
                    form.status === "skipped" ? "opacity-50 grayscale bg-card/30" : "bg-card/60"
                  }`}
                >
                  {/* Card header */}
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-bold text-muted-foreground bg-card px-2 py-0.5 rounded border border-card-border">
                            {idx + 1}
                          </span>
                          <CardTitle className="text-xl">{ex.name}</CardTitle>
                        </div>
                        <div className="flex gap-2 mt-2">
                          <Badge variant="secondary" className="text-xs font-normal bg-card/80">
                            {ex.category}
                          </Badge>
                          <Badge variant="secondary" className="text-xs font-normal bg-card/80">
                            {ex.trackingType}
                            ...
                            <button
                              ...
                              className={`flex items-center gap-1 text-xs px-2 py-0.5 rounded border transition-colors ${
                                openTimers[ex.id]
                                  ...
                                  : "bg-card/80 border-card-border text-muted-foreground hover:text-foreground hover:border-primary/30"
                              }`}
                            >
                      ...
                      <Select
                        value={form.status}
                        onValueChange={(v) => handleExChange(ex.id, "status", v)}
                      >
                  ...
                  {form.status !== "skipped" && (
                    <CardContent className="pt-6">
                      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                        {/* Left: targets + history + form tip */}
                        <div className="md:col-span-5 space-y-3">
                          <div className="bg-primary/5 border border-primary/10 rounded-lg p-3 text-sm flex flex-col gap-2">
                            <div className="flex items-center gap-2 text-primary font-medium">
                              <TargetIcon className="w-4 h-4" /> Target
                            </div>
                            <div className="flex flex-wrap gap-4 text-foreground/80">
                              {ed.targetSets && (
                                <div>
                                  <span className="font-semibold text-foreground">{ed.targetSets}</span> sets
                                </div>
                              )}
                              {ed.targetReps && (
                                <div>
                                  <span className="font-semibold text-foreground">{ed.targetReps}</span> reps
                                </div>
                              )}
                              {ed.targetWeightKg && (
                                <div>
                                  <span className="font-semibold text-foreground">{ed.targetWeightKg}</span> kg
                                </div>
                              )}
                              {ed.targetDurationSeconds && (
                                <div>
                                  <span className="font-semibold text-foreground">{ed.targetDurationSeconds}</span> sec
                                </div>
                              )}
                            </div>
                          </div>
                          ...
                          {prev && (
                            <div className="bg-amber-500/5 border border-amber-500/20 rounded-lg p-3 text-sm flex flex-col gap-2">
                              ...
                              <div className="flex flex-wrap gap-4 text-foreground/80">
                                {prev.actualSets != null && (
                                  <div>
                                    <span className="font-semibold text-foreground">{prev.actualSets}</span> sets
                                  </div>
                                )}
                                {prev.actualReps != null && (
                                  <div>
                                    <span className="font-semibold text-foreground">{prev.actualReps}</span> reps
                                  </div>
                                )}
                                {prev.actualWeightKg != null && (
                                  <div>
                                    <span className="font-semibold text-foreground">{prev.actualWeightKg}</span> kg
                                  </div>
                                )}
                                {prev.actualDurationSeconds != null && (
                                  <div>
                                    <span className="font-semibold text-foreground">{prev.actualDurationSeconds}</span> sec
                                  </div>
                                )}
                                {prev.formQuality && (
                                  <div className="text-muted-foreground">{prev.formQuality} form</div>
                                )}
                              </div>
                            </div>
                          )}
                        ...
                        <div className="md:col-span-7 grid grid-cols-2 sm:grid-cols-4 gap-3">
                          {!timed && (
                            <>
                              <div className="space-y-1.5">
                                ...
                                <Input
                                  type="number"
                                  className="bg-card h-10 text-lg font-medium"
                                  value={form.actualSets}
                                  onChange={(e) => handleExChange(ex.id, "actualSets", e.target.value)}
                                />
                              </div>
                              <div className="space-y-1.5">
                                ...
                                <Input
                                  ...
                                  onChange={(e) => handleExChange(ex.id, "actualReps", e.target.value)}
                                />
                              </div>
                              <div className="space-y-1.5 col-span-2 sm:col-span-2">
                                ...
                                <Input
                                  ...
                                  onChange={(e) => handleExChange(ex.id, "actualWeightKg", e.target.value)}
                                />
                              </div>
                            </>
                          )}
                          {timed && (
                            <div className="space-y-1.5 col-span-2 sm:col-span-4">
                              ...
                              <Input
                                type="number"
                                className="bg-card h-10 text-lg font-medium"
                                value={form.actualDurationSeconds}
                                onChange={(e) =>
                                  handleExChange(ex.id, "actualDurationSeconds", e.target.value)
                                }
                              />
                            </div>
                          )}
                          ...
                          <div className="space-y-1.5 col-span-2">
                            ...
                            <Select
                              ...
                              onValueChange={(v) => handleExChange(ex.id, "formQuality", v)}
                            >
                          ...
                          <div className="col-span-2 flex items-center gap-2 mt-6">
                            ...
                            <Label
                              htmlFor={`pain-${ex.id}`}
                              className="text-sm font-normal text-destructive cursor-pointer"
                            >
                  ...
                  {/* Timer panel — collapsible, mounted only when open */}
                  {hasAnyTimer && openTimers[ex.id] && form.status !== "skipped" && (
                    <CardContent className="border-t border-card-border pt-5 pb-5 bg-black/10">
                      ...
                      <div className="flex flex-wrap gap-8">
                        ...
                        {hasRestSetsTimer && (
                          <TimerWidget
                            ...
                            totalSeconds={ed.restBetweenSetsSec!}
                            mode="rest"
                          />
                        )}
                        {hasRestExTimer && (
                          <TimerWidget
                            ...
                            totalSeconds={ed.restBetweenExercisesSec!}
                            mode="rest"
                          />
                        )}
                      </div>
      ...
      <Card className="bg-card/50 backdrop-blur-sm border-card-border mt-8">
        ...
        <CardFooter className="bg-black/5 dark:bg-black/20 border-t border-card-border p-6 flex justify-end">
          <Button
            size="lg"
            className="w-full sm:w-auto px-8 font-semibold text-lg h-12"
